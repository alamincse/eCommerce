		...
		<div class="col-lg-12">
			...
			<div class="table-responsive">
				<table class="table table-hover">
					...
					<tbody>
						@foreach( $onlineProducts as $online )
							<tr>
								<td><a href="{{ URL::route( 'product-edit', $online->id ) }}">{{ HTML::image( $online->image, 'Picture', array('class' => 'col-lg-6	 col-md-5 col-sm-8 col-xs-12') ) }}</a></td>
								<td>{{ $online->name }}</td>
								<td>{{ $online->model }}</td>
								<td>{{ $online->stock }}</td>
								<td>
									@if( $online->n_price <> 0.00 ) 
										${{ $online->n_price }} <br>
										$<del style="color:#F15A23;">{{ $online->price }}</del>
									@else 
										${{ $online->price }} 
									@endif
								</td>
								<td class="text-center">@if( $online->quantity == null ) 1 @else {{ $online->quantity }} @endif</td>
								<td>No</td>
								<td>{{ $online->status }}</td>
								<td>{{ date('d-m-Y a', strtotime($online->updated_at)) }}</td>
								<td class="text-center">
									<a href="{{ URL::route( 'product-edit', $online->id ) }}" class="btn btn-info ttip" data-toggle="tooltip" title="Edit">
										<i class="fa fa-pencil"></i>
									</a>
									{{ Form::button(trans( '<i class="fa fa-remove"></i>' ), 
										array( 'onclick' =>  'showConfirmDeleteModal(
	                                    "' . URL::route( 'onlineDelete', $online->id) . '")',
								        ...
								        <div class="modal-dialog">
								            <div class="modal-content">
								                ...
								                <div class="modal-footer">
								                    {{ Form::open( array( 'route' => array( 'onlineDelete', $online->id ), 'id' => 'deleteForm', 'method' => 'post') ) }}
